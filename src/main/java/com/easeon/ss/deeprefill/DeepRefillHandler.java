...
//    private static final Map<UUID, Long> lastBlockUseTime = new HashMap<>();
    ...
    public static ActionResult onUseItemAfter(EaseonWorld world, EaseonPlayer player, Hand hand) {
        ...
        if (world.isServer() && !actualItem.isFood()) {
            ...
            logger.info("onUseItemBefore: {}, {}", snapshot.stack.getName().getString(), snapshot.stack.getCount());
            logger.info("onUseItemAfter: {}, {}", actualItem.getName().getString(), actualItem.getCount());

            if (snapshot != null && (actualItem.isEmpty() || DeepRefillHelper.isConsumptionChanged(snapshot.stack, actualItem))) {
//                lastBlockUseTime.put(player.getUuid(), System.currentTimeMillis());
    ...
    public static ActionResult onUseBlockAfter(EaseonWorld world, EaseonPlayer player, Hand hand, EaseonBlockHit hit) {
        ...
        logger.info("check: {}, {},{}", actualItem.isFood(), actualItem.easeonBlock().of(CropBlock.class), farmland.of(Blocks.FARMLAND));
        ...
        if (world.isServer() && (!actualItem.isFood() || (actualItem.easeonBlock().of(CropBlock.class) && farmland.of(Blocks.FARMLAND)))) {
            ...
            logger.info("onUseBlockAfter: {}, {}", snapshot.stack.getName().getString(), snapshot.stack.getCount());
            logger.info("onUseBlockAfter: {}, {}", actualItem.getName().getString(), actualItem.getCount());
    ...
    public static ActionResult onFoodConsumeAfter(EaseonWorld world, EaseonPlayer player, EaseonItem item) {
        logger.info("onFoodConsumeAfter======>");
        if (world.isServer()) {
            var snapshot = foodBeforeConsume.remove(player.getUuid());
            var actualItem = player.getStackInHand(player.get().getActiveHand());
            logger.info("onFoodConsumeBefore: {}, {}", snapshot.stack.getName().getString(), snapshot.stack.getCount());
            logger.info("onFoodConsumeAfter: {}, {}", actualItem.getName().getString(), actualItem.getCount());
            if (snapshot != null && (actualItem.isEmpty() || DeepRefillHelper.isConsumptionChanged(snapshot.stack, actualItem))) {
